...
describe('profile-io path and naming contracts', () => {
  ...
  it('excludes recoverable .profile-backup-* files while listing normal profiles', async (t) => {
    const directory = await temporaryDirectory(t);
    await writeFile(join(directory, 'alpha.json'), '{}');
    await writeFile(join(directory, '.profile-backup-alpha-1699999999999-0.json'), '{}');

    assert.deepEqual(listProfileFiles(directory), ['alpha.json']);
  });
  ...
  it('resolves the bundled directory to the package profiles folder by default', async () => {
    await withEnvironment({ OMO_BUNDLED_PROFILES_DIR: undefined }, async () => {
      ...
      for (const expected of [
        ...
        assert.ok(files.includes(expected), `expected bundled profile ${expected} in ${dir}`);
      }
    });
  });
