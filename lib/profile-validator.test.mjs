...
import {
  ...
  MissingError,
  exitCodeFor,
} from './profile/lifecycle-errors.mjs';
...
describe('validateProfileId', () => {
  it('accepts IDs beginning with an alphanumeric character', () => {
    // Given: supported profile ID characters
    const ids = ['a', 'A9', 'profile.name', 'profile_name', 'profile-name'];

    // When: each ID is validated
    const results = ids.map(validateProfileId);

    // Then: every valid ID is accepted
    assert.deepEqual(results, [true, true, true, true, true]);
  });

  it('rejects empty, dot, traversal, and separator IDs', () => {
    // Given: unsafe or malformed profile IDs
    const ids = ['', '.', '..', '../escape', 'nested/path', 'nested\\path', '-start', '_start'];

    // When: each ID is validated
    const results = ids.map(validateProfileId);

    // Then: no unsafe ID is accepted
...
// ---------------------------------------------------------------------------
describe('lifecycle boundary contracts', () => {
  it('maps typed operational errors to stable exit codes', () => {
    // Given: every supported operational error category
    const errors = [
      new ArgumentError('argument required'),
      new MissingError('profile missing'),
      new ExistsError('profile exists'),
      new InvalidProfileError('profile invalid'),
      new LockUnavailableError('profile locked'),
    ];

    // When: the CLI boundary classifies them
    const exitCodes = errors.map(exitCodeFor);

    // Then: callers receive stable automation-safe codes
    assert.deepEqual(exitCodes, [2, 3, 4, 5, 6]);
  });

  it('creates success JSON envelopes with ok and resource IDs', () => {
    // Given: lifecycle resource identifiers
    const resources = { profileId: 'safe-profile', configPath: '/tmp/config.json' };

    // When: a success response is constructed
    const output = jsonSuccess(resources);

    // Then: machine consumers receive an explicit success marker and identifiers
    assert.deepEqual(output, { ok: true, ...resources });
  });
});
