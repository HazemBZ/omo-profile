...
function runCliTty(args, input) {
  return new Promise(resolve => {
    const child = execFile(
      'script',
      ['-qec', `${process.execPath} ${JSON.stringify(CLI)} ${args.map(JSON.stringify).join(' ')}`, '/dev/null'],
      {
        env: {
          ...process.env,
          OMO_CONFIG_PATH: configPath,
...
describe('agent-profile CLI (integration)', () => {
  ...
  it('list --json: no profiles → success envelope', async () => {
    const r = await runCli(['list', '--json']);
    assert.equal(r.exitCode, 0);
    assert.equal(r.stderr, '');
    assert.deepEqual(JSON.parse(r.stdout), { ok: true, profiles: [] });
  });
  ...
  it('list: bundled profiles never overwrite an existing profile of the same id', async () => {
    writeFileSync(
      join(profilesDir, 'gpt-mix.json'),
      JSON.stringify({ metadata: { name: 'mine' } }) + '\n',
    );
    const r = await runCliWithDefaultBundle(['list']);
    assert.equal(r.exitCode, 0);
    const raw = readFileSync(join(profilesDir, 'gpt-mix.json'), 'utf-8');
    assert.deepEqual(JSON.parse(raw).metadata, { name: 'mine' });
  });

  it('switch --dry-run: works against a bundled profile', async () => {
    const r = await runCliWithDefaultBundle(['switch', 'gpt-mix', '--dry-run']);
    assert.equal(r.exitCode, 0);
  ...
  it('switch: applies a bundled profile and backs up the config', async () => {
    const r = await runCliWithDefaultBundle(['switch', 'gpt-mix']);
    assert.equal(r.exitCode, 0);
    ...
    const backups = readdirSync(tmpDir).filter(f => f.startsWith(`${basename(configPath)}.backup-`));
  ...
  it('current: identifies a bundled profile when the configuration matches', async () => {
    ...
    // which is exactly the bundled deepseek-free profile.
    const r = await runCliWithDefaultBundle(['current']);
    assert.equal(r.exitCode, 0);
    assert.equal(r.stdout, 'Current profile: deepseek-free');
  });
  ...
  it('save: invalid ID exits 2 and writes diagnostics only to stderr', async () => {
    // Given: a traversal-shaped profile ID
    const id = '../escape';

    // When: save runs through the CLI boundary
    const result = await runCli(['save', id]);

    // Then: automation receives invalid-argument exit and no stdout payload
    assert.equal(result.exitCode, 2);
    assert.equal(result.stdout, '');
    assert.ok(result.stderr.includes('alphanumeric'), result.stderr);
  });

  // -----------------------------------------------------------------------
  // lifecycle commands (P3)
  // -----------------------------------------------------------------------
  it('save --force --json: replaces through ProfileStore and reports its backup', async () => {
    // Given: a saved profile and changed active config
    await runCli(['save', 'force-target']);
    const changed = fullValidConfig();
    changed.agents.oracle.model = 'changed/model';
    writeFileSync(configPath, JSON.stringify(changed, null, 2) + '\n');

    // When: force saving through the JSON route
    const result = await runCli(['save', 'force-target', '--force', '--json']);

    // Then: one success envelope identifies the saved profile and backup
    assert.equal(result.exitCode, 0, result.stderr);
    assert.equal(result.stderr, '');
    const response = JSON.parse(result.stdout);
    assert.equal(response.ok, true);
    assert.equal(response.profileId, 'force-target');
    assert.ok(response.backupPath.includes('.profile-backup-force-target-'));
  });

  it('save --force: list never leaks .profile-backup-* files as profiles', async () => {
    // Given: a saved profile then a forced save over it (publishing a backup)
    await runCli(['save', 'alpha']);
    const forced = await runCli(['save', 'alpha', '--force', '--json']);
    assert.equal(forced.exitCode, 0, forced.stderr);
    const backupPath = JSON.parse(forced.stdout).backupPath;
    assert.ok(backupPath.includes('.profile-backup-alpha-'), 'forced save published a backup');
    assert.ok(existsSync(join(profilesDir, basename(backupPath))), 'backup file exists in profiles dir');

    // When: listing in JSON and human form
    const json = await runCli(['list', '--json']);
    const human = await runCli(['list']);

    // Then: only alpha is reported and the backup filename never appears
    assert.equal(json.exitCode, 0, json.stderr);
    assert.deepEqual(JSON.parse(json.stdout).profiles.map(p => p.id), ['alpha']);
    assert.equal(human.exitCode, 0, human.stderr);
    assert.ok(!human.stdout.includes('.profile-backup-'), human.stdout);
  });

  it('clone and rename --json: mutate only ProfileStore profiles', async () => {
    // Given: a source profile
    await runCli(['save', 'source']);

    // When: cloning then renaming through JSON routes
    const cloned = await runCli(['clone', 'source', 'copy', '--json']);
    const renamed = await runCli(['rename', 'copy', 'renamed', '--json']);

    // Then: both responses are machine-readable and source remains intact
    assert.deepEqual(JSON.parse(cloned.stdout), { ok: true, sourceId: 'source', profileId: 'copy' });
    assert.deepEqual(JSON.parse(renamed.stdout), { ok: true, oldId: 'copy', profileId: 'renamed' });
    assert.ok(existsSync(join(profilesDir, 'source.json')));
    assert.ok(existsSync(join(profilesDir, 'renamed.json')));
    assert.equal(existsSync(join(profilesDir, 'copy.json')), false);
  });

  it('rename missing: exits 3 with stderr diagnostic and creates no profiles', async () => {
    // Given: no source or destination profile
    const source = 'missing-source';
    const destination = 'new-destination';

    // When: rename crosses the CLI boundary
    const result = await runCli(['rename', source, destination]);

    // Then: missing error is stderr-only and filesystem stays unchanged
    assert.equal(result.exitCode, 3);
    assert.equal(result.stdout, '');
    assert.ok(result.stderr.includes('does not exist'), result.stderr);
    assert.equal(existsSync(join(profilesDir, `${source}.json`)), false);
    assert.equal(existsSync(join(profilesDir, `${destination}.json`)), false);
  });

  it('delete: non-TTY without --yes exits 2 and leaves profile untouched', async () => {
    // Given: a saved profile
    await runCli(['save', 'remove-me']);

    // When: delete runs in subprocess without confirmation
    const result = await runCli(['delete', 'remove-me']);

    // Then: argument exit preserves profile bytes
    assert.equal(result.exitCode, 2);
    assert.equal(result.stdout, '');
    assert.ok(existsSync(join(profilesDir, 'remove-me.json')));
  });

  it('delete: interactive no leaves profile untouched', {
    skip: process.platform === 'win32'
      ? 'POSIX pseudo-terminal harness requires script(1)'
      : false,
  }, async () => {
    // Given: a saved profile in a pseudo-terminal
    await runCli(['save', 'keep-me']);

    // When: confirmation receives a no response
    const result = await runCliTty(['delete', 'keep-me'], 'n\n');

    // Then: command succeeds without deletion
    assert.equal(result.exitCode, 0, result.stderr);
    assert.ok(existsSync(join(profilesDir, 'keep-me.json')));
  });

  it('delete --yes --json: deletes matching profile without changing active config', async () => {
    // Given: current config saved under one profile
    await runCli(['save', 'active']);
    const before = readFileSync(configPath, 'utf8');

    // When: forced delete uses JSON output
    const result = await runCli(['delete', 'active', '--yes', '--json']);

    // Then: output has one envelope, profile disappears, config bytes remain
    assert.deepEqual(JSON.parse(result.stdout), { ok: true, profileId: 'active' });
    assert.equal(result.stderr, '');
    assert.equal(existsSync(join(profilesDir, 'active.json')), false);
    assert.equal(readFileSync(configPath, 'utf8'), before);
  });

  it('backups, restore, and prune --json: use ConfigStore backup lifecycle', async () => {
    // Given: switching produces an active-config backup
    const target = fullValidConfig();
    target.agents.oracle.model = 'restored/model';
    writeProfile('target', target);
    const before = readFileSync(configPath, 'utf8');
    await runCli(['switch', 'target']);

    // When: listing, restoring, then pruning through JSON routes
    const listed = await runCli(['backups', '--json']);
    const backupId = JSON.parse(listed.stdout).backups[0].id;
    const restored = await runCli(['restore', backupId, '--json']);
    const pruned = await runCli(['backups', 'prune', '--keep', '1', '--json']);

    // Then: each emits only one success envelope and restore exacts prior bytes
    assert.equal(listed.stderr, '');
    assert.equal(JSON.parse(listed.stdout).ok, true);
    assert.deepEqual(JSON.parse(restored.stdout), { ok: true, backupId });
    assert.equal(readFileSync(configPath, 'utf8'), before);
    assert.equal(JSON.parse(pruned.stdout).ok, true);
  });

  it('switch --json: emits one success envelope without human text', async () => {
    // Given: a profile with a changed agent model
    const target = fullValidConfig();
    target.agents.oracle.model = 'json/model';
    writeProfile('json-target', target);

    // When: switch uses JSON output
    const result = await runCli(['switch', 'json-target', '--json']);

    // Then: stdout is one parseable success response and stderr stays empty
    assert.equal(result.exitCode, 0, result.stderr);
    assert.equal(result.stderr, '');
    const response = JSON.parse(result.stdout);
    assert.equal(response.ok, true);
    assert.equal(response.profileId, 'json-target');
    assert.ok(response.backupId);
  });

  it('lifecycle grammar: unknown or duplicate options exit 2', async () => {
    // Given: malformed lifecycle invocations
    const malformed = [
      ['save', 'profile', '--wat'],
      ['clone', 'source', 'destination', '--json', '--json'],
      ['backups', 'prune', '--keep', '0'],
      ['restore'],
    ];

    // When: each crosses CLI grammar boundary
    const results = await Promise.all(malformed.map(args => runCli(args)));

    // Then: all use argument-error exit before mutation
  ...
  it('switch: applies profile with atomic write and preserves top-level keys', async () => {
    ...
    const backups = files.filter(f => f.startsWith(`${basename(configPath)}.backup-`));
  ...
  it('switch: concurrent mutations serialize, retain distinct backups, and release the lock', async () => {
    ...
    writeProfile('first', first);
    writeProfile('second', second);

    const results = await Promise.all([runCli(['switch', 'first']), runCli(['switch', 'second'])]);
    ...
    assert.equal(existsSync(join(tmpDir, '.omo-profile.lock')), false);
    const finalModel = JSON.parse(readFileSync(configPath, 'utf8')).agents.sisyphus.model;
  ...
  // switch errors
  // -----------------------------------------------------------------------
  it('switch ../escape: exits 2 before reading outside profiles dir', async () => {
    const beforeContent = readFileSync(configPath, 'utf-8');
  ...
  it('switch nonexistent: exits 3 with error message', async () => {
    const r = await runCli(['switch', 'no-such-profile']);
    assert.equal(r.exitCode, 3);
    assert.ok(r.stderr.includes('not found'));
  });

  it('switch with invalid profile: exits 5', async () => {
...
`);
  ...
  it('missing config: current exits 3 with "No Oh My OpenAgent configuration found." and "Checked:"', async () => {
  ...
  it('malformed JSONC: current exits 5 with parse error and line info', async () => {
