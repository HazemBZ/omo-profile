...
describe('agent-profile CLI (integration)', () => {
  ...
  it('save→switch round-trip: preserves sparse entries and unknown fields', async () => {
    ...
    assert.equal(currentRes.stdout, 'sparse');
  });

  it('switch --dry-run: model-less entries print (inherit), not undefined', async () => {
    const sparseCfg = {
      $schema: 'https://example.com/schema.json',
    ...
    assert.ok(r.stdout.includes('(inherit)'), `dry-run shows (inherit): ${r.stdout}`);
...
`);
    ...
    assert.equal(r.exitCode, 0, r.stderr);
    assert.equal(r.stdout, 'alias-probe');
  });
});
