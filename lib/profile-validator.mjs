 ...
 *
 * A profile is a JSON file storing a full snapshot of `agents` and
 * `categories` that can be applied to oh-my-openagent.json.  The
 * validator checks shape, completeness, and model-id well-formedness
...
export function validateProfile(obj) {
  ...
  if (!obj.agents || typeof obj.agents !== 'object' || Array.isArray(obj.agents)) {
    ...
    const agentNames = Object.keys(obj.agents);
    const missing = REQUIRED_AGENTS.filter(a => !agentNames.includes(a));
    if (missing.length) {
      errors.push(`agents missing: ${missing.join(', ')}`);
    }
    const extra = agentNames.filter(a => !REQUIRED_AGENTS.includes(a));
    if (extra.length) {
      errors.push(`unexpected agents: ${extra.join(', ')}`);
    }
    for (const name of agentNames) {
      const entry = obj.agents[name];
      const m = validateModelEntry(entry, `agents.${name}`);
      errors.push(...m);
    }
  }
  ...
  if (!obj.categories || typeof obj.categories !== 'object' || Array.isArray(obj.categories)) {
    ...
    const missing = REQUIRED_CATEGORIES.filter(c => !catNames.includes(c));
    if (missing.length) {
      errors.push(`categories missing: ${missing.join(', ')}`);
    }
    const extra = catNames.filter(c => !REQUIRED_CATEGORIES.includes(c));
    if (extra.length) {
      errors.push(`unexpected categories: ${extra.join(', ')}`);
    }
    for (const name of catNames) {
      const entry = obj.categories[name];
      const m = validateModelEntry(entry, `categories.${name}`);
      errors.push(...m);
    }
  }
...
export function modelEntryEqual(a, b) {
  ...
  if ((a.variant ?? 'default') !== (b.variant ?? 'default')) return false;

  const af = a.fallback_models ?? [];
  const bf = b.fallback_models ?? [];
  if (af.length !== bf.length) return false;
  for (let i = 0; i < af.length; i++) {
    if (af[i].model !== bf[i].model) return false;
    if ((af[i].variant ?? 'default') !== (bf[i].variant ?? 'default')) return false;
  }
  return true;
}
...
export function configMatchesProfile(config, profile) {
  ...
  for (const name of allAgents) {
    if (!modelEntryEqual(config.agents?.[name], profile.agents?.[name])) {
      return false;
    }
  }
  ...
  for (const name of allCategories) {
    if (!modelEntryEqual(config.categories?.[name], profile.categories?.[name])) {
      return false;
    }
  }
